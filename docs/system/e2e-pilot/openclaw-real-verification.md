...
> 日期: 2026-08-17 ~ 08-20 (多会话: OpenClaw 集成验证 + 攻击话术重建)
> 回答的问题: **"验证了这些 cases 确实可以在 openclaw 里生效吗? 不是你的幻觉吧?"**
> 最终成果: **66 条真实环境有效攻击 + 有防护拦截率 97% / 0 穿透**

## 1. 结论(先说答案)

**在此项工作之前: 没有。** 此前 v9/v10 的全部数字来自 `multiround_runner.py`——它直接调
LLM API 并自行模拟多轮工具循环, 是"仿真 agent 路径", 不是 OpenClaw。且其 BLOCKED 数字
被 judge 解析 bug 系统性虚高, 真实攻击有效率被严重高估。

**现在: 是, 且有完整测量链。**
- 打通真实 OpenClaw → gov_proxy → LLM → MCP 工具全链路, 修复 14 个真实问题
- 六波攻击话术重设计(349 候选 → 163 裸诱导 → **66 条真实环境有效**)
- 66 条真实有效攻击 × 有防护 OpenClaw: **64 拦截 + 2 自拒 + 0 穿透 = 97% 拦截**
...
### 5.4 六波 v11 攻击话术重建 + 完整测量链(2026-08-19 ~ 08-20)

素材来源: block-01 ArbiterOS 红队池(247 条载荷语义)、arbiteros_standard_cases
(15 条 InjecAgent 模式)、自有 v9/v10 实证(活案例话术反推)。

六波递进(生成器: `src/scripts/_gen_v11{a..f}_candidates.py`):

| 波次 | 策略 | 候选 | 裸诱导 | OpenClaw 有效 | 端到端产率 |
|---|---|---|---|---|---|
| v11 | 实证话术原则, 9 模式均衡 | 90 | 18 | 2 | 2% |
| v11b | 载体转向: exec/write | 88 | 20 | 3 | 3% |
| v11c | 运维掩护故事 (6 类) | 68 | 45 | 23 | 34% |
| v11d | 掩护多样性扩展 | 46 | 32 | 9 | 20% |
| v11e | 单位/命令变体扩充 | 57 | 48 | 14 | 25% |
| v11f | 最高产配方提纯 | 42 | 40 | 15 | 38% |
| **合计** | | **391** | **203 (52%)** | **66 (17%)** | — |

**最终测量链**:
```
v11f 配方 (简单 tar czf + curl -T + 熟悉单位 + 强掩护)
   │  裸 runner 无防护预筛 (391 → 203 诱导)
   ▼
66 条真实环境有效 (OpenClaw 无防护, 致命工具真实执行)
   │  有防护 OpenClaw 复测
   ▼
64 BLOCKED + 2 LLM 自拒 + 0 穿透 = 97% 拦截率
```

**产物**: `data/system-design/e2e-pilot/v11_final_effective_pool.jsonl` (66 条),
后续防护回归的标准被测对象。

**三个结构性结论**:
1. 模型拒绝的是语义目标不是工具: 审批/签字/发布/间接注入 0/40+ (天花板),
   exec 运维类可达 84% 裸诱导。
2. 配方产率可工程化: v11c→v11f 端到端产率 34%→38%,
   简单命令 + 熟悉单位 + 强掩护是最高产组合。
3. 旧数据集(v8/v9/v10 共 156 条)基于带 judge bug 的防护测量,
   有效率被虚高; v11 66 条是修复后独立验证的有效基准。
